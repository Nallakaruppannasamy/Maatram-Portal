...
export const ZoneDashboardPage: React.FC = () => {
  const {
    ...
    queryFn: () => analyticsApi.getZoneDashboard(),
  })
  ...
  if (isLoading) {
    return (
      <div className="space-y-8 animate-in fade-in duration-300">
        <div>
          <h2 className="text-3xl font-extrabold text-[#111827] tracking-tight">Zone Executive Dashboard</h2>
          <p className="text-xs text-[#45464c]">Zone-scoped live analytics and operations.</p>
        </div>
  ...
  if (isError || !data) {
    return (
      <Card className="p-8 text-center space-y-4 max-w-xl mx-auto my-12 bg-red-50/50 border-red-200">
        ...
        <div>
          <h3 className="text-lg font-bold text-red-900">Failed to load Zone Dashboard data</h3>
          <p className="text-xs text-red-700 mt-1">An unexpected error occurred while fetching metrics for your zone.</p>
        </div>
        <Button
          variant="gold"
          size="sm"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#76777d]">Assigned Zone Operations</span>
          </div>
          ...
          <p className="text-xs text-[#45464c]">
            Assigned zone live analytics, student metrics, and volunteering governance.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Link to="/zone/students">
            <Button variant="gold" size="sm" icon={<Users className="w-4 h-4" />}>
              Student Directory
            </Button>
          </Link>
          <Link to="/zone/volunteering-logs">
            <Button variant="outline" size="sm" icon={<HeartHandshake className="w-4 h-4" />}>
              Review Logs
            </Button>
          </Link>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
          <Card className="p-3.5 bg-white border border-[#E5E7EB] rounded-xl flex flex-col justify-between">
            ...
            <span className="text-[10px] font-medium text-blue-600 mt-1">In Zone</span>
          </Card>
          <Card className="p-3.5 bg-white border border-[#E5E7EB] rounded-xl flex flex-col justify-between">
          ...
          <Card className="p-3.5 bg-white border border-[#E5E7EB] rounded-xl flex flex-col justify-between">
            <span className="text-[10px] font-bold text-[#76777d] uppercase tracking-wider">Colleges</span>
          ...
          <Card className="p-3.5 bg-white border border-[#E5E7EB] rounded-xl flex flex-col justify-between">
            <span className="text-[10px] font-bold text-[#76777d] uppercase tracking-wider">SPOCs</span>
          ...
          <Card className="p-3.5 bg-white border border-[#E5E7EB] rounded-xl flex flex-col justify-between">
            <span className="text-[10px] font-bold text-[#76777d] uppercase tracking-wider">Volunteers</span>
          ...
          <Card className="p-3.5 bg-white border border-[#E5E7EB] rounded-xl flex flex-col justify-between">
            <span className="text-[10px] font-bold text-[#76777d] uppercase tracking-wider">Volunteering Logs</span>
            <p className="text-2xl font-black text-indigo-600 mt-1">{volunteeringActivities.total}</p>
            <span className="text-[10px] font-medium text-indigo-600 mt-1">Submissions</span>
          </Card>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <Card className="p-5 bg-white border border-[#E5E7EB] rounded-2xl shadow-sm lg:col-span-3 space-y-4">
          <div className="flex items-center justify-between border-b border-gray-100 pb-3">
            ...
            <span className="text-xs font-bold text-gray-500">Zone Status Breakdown</span>
          </div>
        ...
        <Card className="p-5 bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-2xl shadow-sm flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-[#D4AF37] flex items-center gap-1.5">
                <Percent className="w-4 h-4" /> Zone Conversion
              </span>
              <Badge variant="gold" className="text-[10px]">Active Ratio</Badge>
            </div>
            <h4 className="text-sm font-bold text-slate-200 mt-2">Zone Conversion Rate</h4>
          ...
          <div className="pt-3 border-t border-slate-700/60 text-xs text-slate-300">
            <p>
              ...
              <strong className="text-white">{overview.activeStudents}</strong> active students in this zone earned points.
            </p>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-5 bg-white border border-[#E5E7EB] rounded-2xl shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-black text-[#111827] uppercase tracking-wider">
              Zone Year-wise Student Distribution
            </h3>
          </div>
          <span className="text-xs font-semibold text-gray-500">Active Students in Zone</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-black text-[#111827] uppercase tracking-wider">Zone Performance Highlights</h3>
          </div>
          <Link to="/zone/analytics">
        ...
        {/* Top & Lowest Performing Colleges */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Card className="p-4 bg-gradient-to-br from-blue-50 to-indigo-100/60 border border-blue-200 rounded-2xl space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-black uppercase tracking-wider text-blue-800 flex items-center gap-1">
                <Award className="w-3.5 h-3.5 text-blue-600" /> Top Performing College
              </span>
            ...
            {highlights?.topCollege ? (
              <div>
                ...
                <p className="text-xs font-mono text-blue-800 font-semibold">
                  {highlights.topCollege.collegeCode}
                </p>
              ...
              <p className="text-xs text-gray-500 italic pt-2">No college data available in zone</p>
            )}
          </Card>
          ...
          <Card className="p-4 bg-gradient-to-br from-rose-50 to-rose-100/60 border border-rose-200 rounded-2xl space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="rejected" className="text-[10px]">Attention Needed</Badge>
            </div>
            {highlights?.lowestCollege ? (
              <div>
                ...
                <p className="text-xs font-mono text-rose-800 font-semibold">
                  {highlights.lowestCollege.collegeCode}
                </p>
        ...
        {/* Top 5 Students Portrait Cards */}
        <Card className="p-6 bg-white border border-[#E5E7EB] rounded-2xl shadow-luxury space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-gray-100 pb-4">
            <div>
              <div className="flex items-center gap-2">
                <Trophy className="w-5 h-5 text-amber-500" />
                <h3 className="text-base font-black text-[#111827] tracking-tight">Top 5 Zone Scholars</h3>
              </div>
              <p className="text-xs text-[#76777d] font-semibold mt-0.5">
                Top volunteering scholars within your assigned zone.
              </p>
            </div>
            <Badge variant="gold" className="text-[11px] font-mono px-3 py-1">Zone Top Performers</Badge>
          </div>
          ...
          {(highlights?.topStudents || []).length === 0 ? (
            <div className="py-12 text-center space-y-2 bg-gray-50/60 rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-xs font-bold text-gray-700">No approved volunteering activity in zone</p>
              <p className="text-[11px] text-gray-400">Students in your zone have not logged approved volunteering yet.</p>
            </div>
          ) : (
