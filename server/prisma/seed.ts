...
async function main() {
  ...
  await prisma.volunteerSubmission.create({
    data: {
      ...
      category: VolunteerCategory.OTHER_OFFLINE_EVENT_VOLUNTEERING,
  ...
  await prisma.volunteerSubmission.create({
    data: {
      ...
      category: VolunteerCategory.PHYSICAL_VERIFICATION,
      count: 15,
  ...
  await prisma.volunteerSubmission.create({
    data: {
      ...
      description: 'Conducted a candidate screening for student selection.',
      status: VolunteerStatus.pending,
    },
  });
